...
void jouer (SDL_Surface *screen,int *stage,perssonage p,background b,int loadtest)
{
...
SDL_Rect relcoord_perssonage={p.rect.x+b.x_relative,poskey3.y};		
...
SDL_Rect relcoord6={f.rect.x-b.x_relative,f.rect.y};	
...
afficher_enemie (&f,screen,relcoord6);
animation (&f);			
...
if (collision(&relcoord6,&p.rect) && !p.invincible)
{
p.vie--;
p.invincible=true;
}
